...
export default function SettingsPage() {
    ...
    const fetchAppSettings = async () => {
        ...
        if (data) {
            data.forEach(item => {
                if (item.key === 'admin_email') setAdminEmail(item.value);
                if (item.key === 'account_details') setSettings(prev => ({ ...prev, account_details: item.value }));
                if (item.key === 'hourly_labor_rate') setSettings(prev => ({ ...prev, hourly_labor_rate: Number(item.value) }));
            });
        }
    };
    ...
    const handleRenameCategory = async (oldName: string) => {
        ...
        setModalConfig({
            ...
            message: `Rename category "${oldName}" to "${newCategoryName}"? This will update all items in this category.`,
            type: "info",
            onConfirm: async () => {
    ...
    const handleSave = async () => {
        ...
        // Save App Settings (Admin Email & Extensions)
        const updates = [
            { key: 'admin_email', value: adminEmail },
            { key: 'account_details', value: settings.account_details },
            { key: 'hourly_labor_rate', value: settings.hourly_labor_rate?.toString() || "0" }
        ];
        ...
        // Exclude schema-missing fields and ID (generated by DB)
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { account_details, hourly_labor_rate, id, ...settingsToSave } = settings;

        let error;

        // Use UPDATE if we have an ID (which we should from fetch), otherwise INSERT
        // Note: For 'settings' which is a singleton, we typically update the known row.
        if (settings.id) {
